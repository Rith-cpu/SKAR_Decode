...
@Disabled
@Autonomous(name = "AutoRedFarFar",group = "Red")
public class AutoRedFar extends LinearOpMode {
    ...
    private void shootPreloads(MecanumDrive drive, TSHI tshi, TurretTesting tt, LimelightDistance ld) {
        ...
        tshi.AutoShooter("start", ld.estimatePowerFromDistanceGreaterThan100(distance) - 30);
    ...
    private void intakeBalls(MecanumDrive drive, TSHI tshi, TurretTesting tt, LimelightDistance ld) {
        Pose2d pos = new Pose2d(14, -60, Math.toRadians(0));

        tshi.AutoIntakeandShoot("intake", 1);

        Actions.runBlocking(
                drive.actionBuilder(pos)
                        .strafeTo(new Vector2d(40, -38),new TranslationalVelConstraint(100), new ProfileAccelConstraint(-90, 90))
                        .build());

        sleep(500);

        Pose2d pos2 = new Pose2d(40, -38, Math.toRadians(0));

        Actions.runBlocking(
                drive.actionBuilder(pos2)
                        .strafeTo(new Vector2d(65, -38), new TranslationalVelConstraint(100), new ProfileAccelConstraint(-20, 90))
                        .build());

        tshi.AutoIntakeandShoot("stop", 0);


       Pose2d pos3 = new Pose2d(65, -38, 0);

        Actions.runBlocking(
                drive.actionBuilder(pos3)
                        .strafeTo(new Vector2d(14, -38), new TranslationalVelConstraint(100), new ProfileAccelConstraint(-90,90))
    ...
    private void shoot(MecanumDrive drive, TSHI tshi, TurretTesting tt, LimelightDistance ld) {
        ...
        Double distance = ld.getDistanceIfValid();
        tshi.AutoTurret(-425);
        tshi.AutoShooter("start", ld.estimatePowerFromDistanceGreaterThan100(distance) - 10); // if 14 put 59
    ...
    private void intakeBalls2(MecanumDrive drive, TSHI tshi, TurretTesting tt, LimelightDistance ld) {
        Pose2d pos = new Pose2d(14, -57, Math.toRadians(0));

        tshi.AutoIntakeandShoot("intake", 1);

        Actions.runBlocking(
                drive.actionBuilder(pos)
                        .strafeToLinearHeading(new Vector2d(40, -18), Math.toRadians(0))
                        .build());

        sleep(500);

        Pose2d pos2 = new Pose2d(40, -18, Math.toRadians(0));

        Actions.runBlocking(
                drive.actionBuilder(pos2)
                        .strafeToLinearHeading(new Vector2d(65, -18), 0)
                        .build());

        tshi.AutoIntakeandShoot("stop", 0);

        Pose2d pos3 = new Pose2d(65, -18, Math.toRadians(0));
    ...
    private void shootAgain(MecanumDrive drive, TSHI tshi, TurretTesting tt, LimelightDistance ld) {
        ...
        Actions.runBlocking(
                drive.actionBuilder(pos)
                        .strafeTo(new Vector2d(20, -57),new TranslationalVelConstraint(30), new ProfileAccelConstraint(-20, 90))
                        .build());
